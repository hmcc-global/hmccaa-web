...
function processEvents(e) {
  return e.map(event => {
    ...
    let description = (event.DescriptionOverride || event.EventTemplate?.Description || "").trim().split('\n');
...
function processEventTimes(times) {
  let recur = (currTime, { recurTimeFrame, recurX, end }) => {
    ...
    while (!end || end >= currTime.start) {
      ...
      currTime = {
        ...
        end: currTime.end ? util.DATE_ITERATOR[recurTimeFrame](currTime.end, recurX) : null,
        showWhenPast,
      }
    }
  ...
  return (times || []).map((time) => {
    ...
    return null;
  }).flat().sort((a, b) => a.start === b.start ? a.end - b.end : a.start - b.start);
}

function filterEventTimes(events) {
  return events.map(event => {
    if (event.times) {
      event.times = event.times.filter(time => time.showWhenPast || util.isTodayOrAfter(time.end ? time.end : time.start)).slice(0, event.showXUpcoming);
    }
    else {
      event.times = [];
    }
